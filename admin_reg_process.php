...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO admin_details (collegeCode, name, contact, email, staffType, designation, password, desig_abb, staff_abb, userID) 
            VALUES ('$collegeCode', '$name', '$contact', '$email', '$staffType', '$designation', '$password', '$desig_abb', '$staff_abb', '$userID')";

    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('Registration successfull');window.location.href='admin_reg.php';</script>";
    } else {
        echo "<script>alert('Registration unsuccessfull');window.location.href='admin_reg.php';</script>";
    }
